...
//                            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Chào con, con muốn ta giúp gì nào?", "Giải tán bang hội", "Nhận quà\nđền bù");
                            ...
                            this.createOtherMenu(player, ConstNpc.BASE_MENU, "Chào con, con muốn ta giúp gì nào? bdkb mở từ cả ngày up hồng ngọc trong đó",
                            "Giải tán bang hội", "Lãnh địa Bang Hội", "Kho báu dưới biển","Đổi đệ tử");
                        }
                    }
                }
            }
...
//                                        throw new RuntimeException(e);
                            ...
                            case 3:    
                                this.createOtherMenu(player, ConstNpc.MENU_DOI_DT, "Con có thể lấy 300 thỏi vàng để nhận được 1 đổi đệ tử",
                                    "Xác nhận đổi","Đóng");
                            break; 
                        }
                    } 
            } if (player.iDMark.getIndexMenu() == ConstNpc.MENU_DOI_DT) {
                            switch (player.combineNew.typeCombine) {
                                case 0: {
                                    Item tv = null;
                                    try {
...
//                                        throw new RuntimeException(e);
                                    ...
                                    if (tv == null || tv.quantity < 300) {
                                        ...
                                        Item dtt = ItemService.gI().createNewItem((short) 401);
                                        InventoryServiceNew.gI().addItemBag(player, dtt);
                                        InventoryServiceNew.gI().sendItemBags(player);
                                        this.npcChat(player, "Bạn nhận được Đổi đệ tử");
                                    }
                                    break;
                                }
                            }
                    ...
                    if (player.iDMark.isBaseMenu()) {
                        switch (select) {
                            ...
                            case 4:
                                ...
                                if(player.getSession().coin < 20000){
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    createOtherMenu(player, ConstNpc.BASE_MENU,
                            ...
                            "Cửa hàng", "Đổi Tiền", "Shop Hồng Ngọc", "Menu mua đệ\nsiêu vip");
                }
            }
                    ...
                    if (this.mapId == 5 || this.mapId == 13 || this.mapId == 20) {
                        if (player.iDMark.isBaseMenu()) {
                            switch (select) {
                                ...
                                case 1:
                                    this.createOtherMenu(player, ConstNpc.QUY_DOI, "|7|Số tiền của bạn còn : " + player.getSession().coin + "\n"
                                            + "Tỉ lệ quy đổi:\n" +" Hồng ngọc 1-1; Thỏi Vàng 1-3\n" + "Muốn quy đổi không", "Quy đổi\n Hồng ngọc", "Quy Đổi Thỏi Vàng");
                                    break;
                            ...
                            try (Connection con = GirlkunDB.getConnection();) {
                                ...
                                ps.setInt(1,  player.getSession().coin);
                    ...
                    if (this.mapId == 46) {
                        if (!TaskService.gI().checkDoneTaskTalkNpc(player, this)) {
                            this.createOtherMenu(player, ConstNpc.BASE_MENU,"\nTa sẽ giúp ngươi chuyển sinh nếu ngươi\nđã đạt tối đa sức mạnh"                                                                         
                                    ...
                                    + "\nvà ngươi sẽ nhận được 500sdg + 10k ki,hpg"
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    createOtherMenu(player, ConstNpc.BASE_MENU,
                            ...
                            "Tóp Sức Mạnh", "Top Nhiệm Vụ", "Top Pvp", "Tóp Ngũ Hành Sơn","Top Chuyển Sinh","Top Nap", "Đóng");
                }
            }
    ...
    public static Npc vihop(int mapId, int status, int cx, int cy, int tempId, int avartar) {
        return new Npc(mapId, status, cx, cy, tempId, avartar) {
            ...
            public void openBaseMenu(Player player) {
                if (canOpenNpc(player)) {
                    createOtherMenu(player, ConstNpc.BASE_MENU,
                            "|2|Đưa 5000 Đồng Xu Vàng Cho Ta \b|7|Ngươi sẽ có được thứ mà người người thèm khát",
                            "Đổi Đồng Xu Vàng", "Quy Đổi Mảnh Hồn", "Quy Đổi Đá Xanh Lam", "Quy Đổi Đồng Xu Vàng", "Shop Sự Kiện","Đổi Đồ\nHủy Diệt", "Đóng");
                }
            }
                    ...
                    switch (this.mapId) {
                        case 5:
                            switch (player.iDMark.getIndexMenu()) {
                                case ConstNpc.BASE_MENU:
                                    if (select == 0) {
                                        Input.gI().createFormTradeRuongDongVang2(player);
                                        break;
                                    }
                                    if (select == 1) {
