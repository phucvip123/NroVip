...
public class NapVangService {
    ...
    public static void ChonGiaTien(int chon, Player p) throws Exception{
        switch (chon){
            case 20: {//(20k)
                if (p.getSession().coin < 20000) {
                    Service.gI().sendThongBao(p, "Số tiền tối thiểu: là 20,000 coin");
                    return;
                }
                ...
                if (thoivang != null) {
                    p.getSession().coin -= 20000;
                    InventoryServiceNew.gI().addItemBag(p, thoivang);
                    InventoryServiceNew.gI().sendItemBags(p);
                    GirlkunDB.executeUpdate("update account set coin = '" + p.getSession().coin + "' where id = " + p.getSession().userId);
            ...
            case 50: {
                if (p.getSession().coin < 50000) {
                    Service.gI().sendThongBao(p, "Số tiền tối thiểu: là 50,000 coin");
                    return;
                }
                if (InventoryServiceNew.gI().getCountEmptyBag(p) == 0) {
                    Service.gI().sendThongBao(p, "Hành trang không đủ chỗ trống");
